...
impl<'a> ThemeRepository<'a> {
    ...
    pub async fn load_all(&self) -> AppResult<Vec<ThemeDto>> {
        ...
        let rows = sqlx::query(
            "SELECT id, name, content, createdAt FROM CustomTheme"
        )
        .fetch_all(pool)
        .await
    ...
    pub async fn save(&self, id: Option<String>, name: &str, css: &str) -> AppResult<String> {
        ...
        if let Some(id_str) = id {
            let id_int = id_str.parse::<i32>().map_err(|_| crate::error::AppError::InvalidRequest("Invalid ID".into()))?;
            sqlx::query(
                "UPDATE CustomTheme SET content = ?, createdAt = ? WHERE id = ?"
            )
            .bind(css)
            .bind(now)
            .bind(id_int)
            ...
            let result = sqlx::query(
                "INSERT INTO CustomTheme (name, content, createdAt) VALUES (?, ?, ?)"
            )
            .bind(name)
            .bind(css)
            .bind(now)
    ...
    pub async fn get(&self, id: i32) -> AppResult<Option<Theme>> {
        let pool = self.db.pool().await?;
        let row = sqlx::query(
            "SELECT id, name, content, createdAt FROM CustomTheme WHERE id = ?"
        )
        .bind(id)
        .fetch_optional(pool)
        .await
